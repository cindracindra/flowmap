...
def analyse(graph: Graph):
    return execution_phase_analysis(graph, build_method_definitions(graph))
...
def test_builds_method_phase_evidence_without_flattening() -> None:
    ...
    assert request["schemaVersion"] == "execution-phase-label-v2"
    assert len(request["subjects"]) == 1
    subject = request["subjects"][0]
    assert subject["id"] == "item-1"
    phase = subject["phaseEvidence"][0]
    assert phase["phaseId"] == "entry:phase:1"
    assert phase["method"] == "Order.checkout:void()"
    assert phase["phaseIndex"] == 1
    assert phase["localPhaseCount"] == 1
    assert phase["coreSignature"] == {
        "receivers": ["order"],
        "domainTypes": ["order"],
        "methodTerms": ["order", "validate"],
    }
    assert phase["operations"][0] == {
        "callee": "Order.validate:void()",
        "code": "order.validate()",
    }
...
def test_groups_transitive_standalone_one_phase_delegates() -> None:
    ...
    subject = request["subjects"][0]
    assert subject["id"] == "item-1"
    assert [phase["phaseId"] for phase in subject["phaseEvidence"]] == [
        "a-entry:phase:1", "b-entry:phase:1", "c-entry:phase:1",
    ]
    assert [phase["method"] for phase in subject["phaseEvidence"]] == [
        "A.run:void()", "B.run:void()", "C.run:void()",
    ]
...
def test_does_not_group_a_call_that_joined_other_caller_operations() -> None:
    ...
    assert all(
        len(subject["phaseEvidence"]) == 1 for subject in request["subjects"]
    )
...
def test_attaches_subject_label_to_every_equivalent_phase_id() -> None:
    ...
    assert [
        ...
        for method in analysis.analyses_by_entry_id.values()
